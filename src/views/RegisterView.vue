  ...
  <div class="auth-page">
    ...
    <div class="auth-left">
      ...
      <div class="left-content">
        <p class="left-eyebrow">Nový účet</p>
        <h1 class="left-heading">Vytvorte<br />si účet.</h1>
        <p class="left-sub">Bezplatná registrácia za pár sekúnd.<br />Jeden účet pre celý AIWai ekosystém.</p>

        <div class="left-badges">
          <span class="badge">Zadarmo navždy</span>
          <span class="badge">Bez kreditky</span>
      ...
      <div class="auth-form-wrap">
        ...
        <div v-if="registered" class="success-state">
          <div class="success-icon">
            <PhCheckCircle :size="52" weight="thin" />
          </div>
          <h2 class="success-title">Účet vytvorený!</h2>
          <p class="success-text">
            Skontroluj svoju e-mailovú schránku a potvrď registráciu kliknutím na odkaz v e-maile.
          </p>
          <router-link to="/login" class="submit-btn" style="margin-top: 1.75rem; text-decoration: none;">
            Prihlásiť sa →
          </router-link>
        </div>
        ...
        <template v-else>
          <div class="form-header">
            ...
            <p class="form-sub">Zadajte údaje pre vytvorenie účtu.</p>
          </div>
            ...
            <div class="field-group">
              ...
              <div class="strength-bar" v-if="password">
                <div
                  v-for="i in 4" :key="i"
                  class="strength-seg"
                  :class="{ active: passwordStrength >= i, strong: passwordStrength === 4 }"
                ></div>
              </div>
            </div>
...
import { PhEye, PhEyeSlash, PhWarning, PhCheckCircle } from '@phosphor-icons/vue'
...
const translateError = (msg) => {
  ...
  if (msg.includes('already registered') || msg.includes('already been registered')) return 'Tento e-mail je už zaregistrovaný'
  if (msg.includes('Password should be')) return 'Heslo musí mať aspoň 6 znakov'
...
.left-note a:hover { text-decoration: underline; }
...
.pass-input { padding-right: 2.75rem; }
...
.strength-seg.active { background: var(--accent-gold); }
.strength-seg.active.strong { background: #4caf50; }
...
@media (max-width: 768px) {
  ...
  .left-sub, .left-note { display: none; }
